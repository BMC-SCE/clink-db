...
int clink_parse_c_l(clink_iter_t **it, const char *filename, const char *compiler, size_t argc, const char **argv) {
  ...
  if (rc) {
    ...
    state_free(&s);
    asprintf(&log_output, "%-9s parse %s - FAILED\n", compiler, filename);
    write_log((unsigned char*)log_output, strlen(log_output), "clink-db.log", false);
  } else {
    *it = wrapper;
    asprintf(&log_output, "%-9s parse %s - OK\n", compiler, filename);
    write_log((unsigned char*)log_output, strlen(log_output), "clink-db.log", false);
  }
...
int clink_parse_c_f(clink_iter_t **it, const char *filename, const char *compiler, size_t argc, const char **argv) {
  ...
  char *tmp_ast=NULL;
  rc=temp_file(&tmp_ast, filename);
  assert(rc == 0);

  // create a clang/clang++ index
  static const int excludePCH = 0;
  static const int displayDiagnostics = 0;
  s->index = clang_createIndex(excludePCH, displayDiagnostics);

  //generate ast file
  if (( rc = clink_compiler_ast(compiler, filename, argc, argv, tmp_ast)))
    goto done;

  if (is_fileRWNZ(tmp_ast)) {
    asprintf(&log_output, "%-9s parse %s - OK\n", compiler, filename);
    write_log((unsigned char*)log_output, strlen(log_output), "clink-db.log", false);
  }
  else {
