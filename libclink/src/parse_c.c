...
int clink_parse_c_l(clink_iter_t **it, const char *filename, size_t argc,
    const char **argv) {
...
int clink_parse_c_f(clink_iter_t **it, const char *filename, size_t argc,
    const char **argv) {

  if (strlen(clang_path) == 0)
    findX_in_PATH ("clang", clang_path, PATH_MAX); 
  
  assert (strlen(clang_path) > 0);
  ...
  //generate temporary ast file
  char *tmp_ast=NULL;
  rc=temp_file(&tmp_ast);
  assert(rc == 0);
  ...
  //generate ast file
  if (( rc = clink_compiler_ast("clang", filename, argc, argv, tmp_ast)))
    goto done;

  // parse the input file
   enum CXErrorCode err = clang_createTranslationUnit2( s->index, tmp_ast, &s->tu );
  if (err != CXError_Success) {
    rc = clang_err_to_errno(err);
    goto done;
  }
