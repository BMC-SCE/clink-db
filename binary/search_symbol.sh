...
cat << EOH
Usage:
        $procname -f DB (clink-db.sqlite3) -n name (optional, symbol name) -t cx_category (optional, one or more numbers comma separated whithout spaces, run "clink-db -H" for all CXCursorKind)
EOH
exit 0
}
