...
static int process(unsigned long thread_id, pthread_t *threads, clink_db_t *db,
  ...
  for (;;) {
    ...
    switch (t.type) {
	  ...
	  if (option.full_ast)
            rc = clink_parse_c_f(&it, t.path, option.cxx_argc, argv);
	  else
            rc = clink_parse_c_l(&it, t.path, option.cxx_argc, argv);

        }
        
	if (rc) {
          free(t.path);
          error(thread_id, "clang/libclang failed to parse %s: %s", display, strerror(rc));
	  continue;
	}
        ...
        if (rc == 0) {
          ...
          while (clink_iter_has_next(it)) {
	        ...
	        error(thread_id, "failed to make %s absolute: %s\n", symbol->path, strerror(errno));
              if ( regexec(&rp, resolved_path, (size_t) 0, NULL, 0) == 0 ){            
